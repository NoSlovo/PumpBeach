...
public class Tower<T> : MonoBehaviour where T : MonoBehaviour
{
   private float _itemHeight = 0.20f;
   private Stack<T> _colection = new Stack<T>();
   private Transform _objectClass ;
   ...
   public void PutElement(T collectable)
   {
      ...
      _currentCount++;
      collectable.transform.SetParent(_objectClass.transform);
